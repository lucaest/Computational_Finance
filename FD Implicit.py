## Finite Difference Implicit Option Pricing
...
def BS_EuCall_FiDi_Explicit(r, sigma, a, b, m, nu_max, T, K):
    ...
    alpha = np.ones(m-1) * (1 + 2*lbd)
    beta = np.ones(m-1)  * (-lbd)
    gamma = np.ones(m-1) * (-lbd)
    ...
    for i in range(1, nu_max):
        for j in range(1, m):
            w[j, i] = -lbd * w[j+1, i-1] + (1 + 2*lbd) * w[j+1, i] - lbd * w[j+1, i+1]
        w[1:-1, i] = TriagMatrix_Inversion(alpha, beta, gamma, w[1:-1, i-1])
    ...
    V = K * w[1:-1, nu_max-1] * np.exp(-x[1:-1] / 2 * (q - 1) - sigma ** 2 / 2 * T * ((q - 1) ** 2 / 4 + q))
    return [S, V]


### test parameter
r = 0.05
sigma = 0.2 #try stability conditon with higher sigma, in contrast to explicit scheme the implicit scheme doesnt start oscillating
a = -0.7
b = 0.4
m = 100
nu_max = 2000
